...
- VOTE: 「一番イケてない回答」に投票（回答者は匿名表示）
- RESULT: ラウンド最少得票の回答者へ `-1` 点
- 同率最少得票: 対象者全員を減点
- RESULT表示: ラウンドビリと総合ビリをバッジで可視化
...
- 共有状態に `activeMemberIds` があり、ラウンド参加対象を管理（ANSWER中の途中参加は当該ラウンドから参加可、VOTE/RESULT中は次ラウンドから）
...
- リロード時の誤退出を防ぐため、pending leave + grace で離脱確定を遅延
...
- Liveblocks本番運用の仕上げ（認証APIの運用固定、障害時方針、監視）
- SQLite編集UI（必要であれば）
